...
解读流水线（核心流程，②③④ 三个子 agent 并行执行）：

```
                 输入：arXiv ID / PDF 上传 / 自然语言描述
                                     │
                                     ▼
┌────────────────────────────────────────────────────────────────────────┐
│                     ① load 准备资料（工作流节点）                      │
│        定位论文 → 章节拆分（按需读取，省上下文）→ 提取原文图表         │
│ → 查找 GitHub 仓库 → 浅克隆到工作区并分析（README/目录/依赖/核心文件） │
└────────────────────────────────────────────────────────────────────────┘
 三路并行（fan-out）
              │                      │                      │
              ▼                      ▼                      ▼
   ...
   ├────────────────────┤ ├────────────────────┤ ├────────────────────┤
   │ 工具：             │ │ 工具：             │ │ 工具：             │
   │  web_search 联网   │ │  grep_repo 搜代码  │ │  read_section      │
   │  search_library    │ │  list_repo_dir     │ ├────────────────────┤
   │  read_section      │ │  read_repo_file    │ │ 输出：             │
   ├────────────────────┤ │  read_section      │ │  结果对比/消融     │
   │ 输出：             │ ├────────────────────┤ │  实验分析报告      │
   │  数据集/baseline   │ │ 输出：             │ └────────────────────┘
   │  背景调研报告      │ │  创新点/公式解释   │
   └────────────────────┘ │  实现细节报告      │
              ...
              └──────────────────────┴──────────────────────┘
                                     ...
                                     ▼
┌────────────────────────────────────────────────────────────────────────┐
│              ⑤ report 报告 agent（等待三路完成，fan-in）               │
│       综合三份子报告 + 原文（read_section 回查）→ 6 章节中文报告       │
└────────────────────────────────────────────────────────────────────────┘
                                    ▼
┌────────────────────────────────────────────────────────────────────────┐
│                   输出：论文工作区 workspaces/{id}/                    │
│   report.md（最终报告）· background/method/experiment.md（阶段结果）   │
│                repo/（代码克隆）· figures/（原文图表）                 │
└────────────────────────────────────────────────────────────────────────┘
```
...
```
入库：arXiv ID → 元数据 → 下载 PDF → 提取文本 → 分块 → Milvus BM25 索引 → 元数据增强 → 入库 ✓
问答：问题 → BM25 混合检索 → answer agent（可追问检索）→ 回答 + 引用来源
创新：勾选论文 → 抽取创新点（校验重试）→ 组合生成想法（引用硬校验）→ 想法入库
```

> 更完整的分层架构与每个子 agent 的工作流（Mermaid 版）见 [ARCHITECTURE.md](ARCHITECTURE.md)。
...
├── pdfs/          # 论文 PDF
├── workspaces/    # 每篇论文独立工作区：repo/（代码克隆）· figures/（图表）· 报告与各阶段结果
├── cache/         # arXiv API 缓存
├── clones/        # 未关联论文的仓库克隆（共享兜底）
...
└── app.py          # FastAPI 单页界面（REST + SSE 流式）
```
